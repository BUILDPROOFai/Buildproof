    ...
    <>
      ...
      <main>
        <section className="relative overflow-hidden">
          <div
            className="absolute -top-32 -right-40 w-[36rem] h-[36rem] rounded-full opacity-[0.15] blur-3xl pointer-events-none"
            style={{ background: "radial-gradient(circle, #3457E0 0%, transparent 70%)" }}
          />
          <div
            className="absolute -top-10 left-1/3 w-[28rem] h-[28rem] rounded-full opacity-[0.10] blur-3xl pointer-events-none"
            ...
            <div className="grid md:grid-cols-[1fr_auto] gap-10 items-center">
              <div>
                ...
                <div className="flex flex-wrap gap-3 mt-9">
                  <a
                    href="#projects"
                    className="bg-seal text-white px-7 py-3.5 rounded-full font-medium text-sm hover:bg-seal/90 transition-colors"
                  >
                    Browse projects
                  </a>
                  <a
                    href="mailto:Offplanadvisory@gmail.com?subject=Get%20my%20project%20verified"
                    className="border border-line bg-white px-7 py-3.5 rounded-full font-medium text-sm text-ink hover:border-ink/20 transition-colors"
                  >
          ...
          <div className="max-w-5xl mx-auto px-6 py-20">
            ...
            <div className="flex flex-wrap gap-3">
              {LEVELS.map((level, i) => (
                <div
                  key={level.n}
                  className={`flex items-center gap-2.5 px-5 py-3 rounded-full border text-sm ${
                    i === LEVELS.length - 1
        ...
        <section className="border-t border-line">
          <div className="max-w-5xl mx-auto px-6 py-16 grid grid-cols-2 sm:grid-cols-4 gap-8">
            {stats.map((s) => (
              <a key={s.label} href={s.href} className="block group">
